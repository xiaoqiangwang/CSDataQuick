    ...
    get_element(int opnd, const char **ppsrc, const ELEMENT **ppel)
{
    ...
    *ppel = NULL;
	...
	if (strncmp(*ppsrc, pel->name, len) == 0) {
	    *ppel = pel;
	    *ppsrc += len;
	    return TRUE;
	}
	    ...
	    *pout++ = pel->code;
        ...
        case LITERAL_OPERAND:
            ...
            if (pel->code == LITERAL_DOUBLE) {
                ...
                lit_ui = (unsigned int) strtoul(psrc, &pnext, 0);
                if (pnext == psrc) {
                    *perror = CALC_ERR_BAD_LITERAL;
                    goto bad;
                }
                psrc = pnext;
                *pout++ = LITERAL_INT;
                memcpy(pout, &lit_ui, sizeof(unsigned int));
